...
CUDA_DRIVER_HANDLER(LaunchKernel) {
    int gridDimX = input_buffer->Get<int>();
    int gridDimY = input_buffer->Get<int>();
    int gridDimZ = input_buffer->Get<int>();
    int blockDimX = input_buffer->Get<int>();
    int blockDimY = input_buffer->Get<int>();
    int blockDimZ = input_buffer->Get<int>();
    int sharedMem = input_buffer->Get<int>();
    CUfunction f = input_buffer->Get<CUfunction > ();
    CUstream hStream = input_buffer->Get<CUstream > ();
    //void *kernelParams = input_buffer->Assign<void *> ();
    //void *extra = input_buffer->Assign<void *> ();
    CUresult exit_code = cuLaunchKernel(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ,
    		sharedMem, hStream, NULL, NULL);
    return new Result((cudaError_t) exit_code);
}
